...
function main()
    ...
    # Make sure to update the registry as well
    Pkg.Registry.update()
    # Update dependencies, without recompiling them
    ...
    # # Create new branch, commit, push, and open pull request
    # repo = LibGit2.GitRepo(dirname(@__DIR__))
    # branch = "gha/bump-manifest/" * Dates.format(now(UTC), dateformat"YYYYmmddTHHMMSS")
    # LibGit2.branch!(repo, branch)
    # LibGit2.add!(repo, ".ci/Manifest.toml")
    # LibGit2.commit(repo, msg)
    # # LibGit2.push(
    # #     repo;
    # #     refspecs=["refs/heads/" * branch],
    # #     remoteurl="https://github.com/JuliaPackaging/Yggdrasil.git",
    # #     credentials=creds,
    # # )
